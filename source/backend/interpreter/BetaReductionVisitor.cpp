...
#include "AlphaRenamingVisitor.h"

backend::interpreter::BetaReductionVisitor::BetaReductionVisitor() {
    // Nothing to replace
    ast::scope_reference scope = std::make_shared<ast::Scope>();
    replacement_table = scope;
}
backend::interpreter::BetaReductionVisitor::BetaReductionVisitor(ast::scope_reference _replacement_table) {
    replacement_table = _replacement_table;
}



ast::node_reference backend::interpreter::BetaReductionVisitor::visitVariable(std::shared_ptr<ast::Variable> variable) {
...
ast::node_reference backend::interpreter::BetaReductionVisitor::visitAssignment(std::shared_ptr<ast::Assignment> assignment) {
    return (ast::node_reference)assignment;
}
ast::node_reference backend::interpreter::BetaReductionVisitor::visitApplication(std::shared_ptr<ast::Application> application) {
    ast::node_reference app = std::make_shared<ast::Application>(application->lhs->accept(this), application->rhs->accept(this));
    return app;
}
ast::node_reference backend::interpreter::BetaReductionVisitor::visitAbstraction(std::shared_ptr<ast::Abstraction> abstraction) {
    if (replacement_table->has(abstraction->argument)) {
        ...
        backend::interpreter::AlphaRenamingVisitor alpha_renamer(rename_table);
...
std::string backend::interpreter::BetaReductionVisitor::enumerate_name(const std::string &name) {
    int suffix = 0;
    std::string renamed;
    do {
